# -*- coding: utf-8 -*-
"""
AP 时间感受器模块 — 模块专属日志管理器
======================================

说明：
  - 参考 action/_logger.py 的实现，保持同一套“三层日志”口径：
    error / brief / detail
  - 原型阶段强调“可观测性与可审计”，因此日志字段中文优先，必要时附英文解释。
...
class ModuleLogger:
    ...
    def _get_or_open(self, level_key: str, target_dir: Path):
        ...
        if fh and not fh.closed:
            return fh
        path = target_dir / f"{_MODULE_NAME}_{level_key}.log"
    ...
    def _rotate(self, level_key: str, target_dir: Path) -> None:
        ...
        base = target_dir / f"{_MODULE_NAME}_{level_key}.log"
        if not base.exists():
            return
        ts = time.strftime("%Y%m%d_%H%M%S")
        rotated = target_dir / f"{_MODULE_NAME}_{level_key}_{ts}.log"
        try:
            base.rename(rotated)
        except OSError:
            pass
