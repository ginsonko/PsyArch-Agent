...
echo ======================================
echo   PsyArch-Agent data migration restore
echo ======================================
echo Folder: %cd%
echo.
echo This restores the migration zip into this PA folder.

echo Existing target files are backed up first.
echo.
...
echo.
if not "%CODE%"=="0" (
  echo [PA] Restore helper exited with errorlevel=%CODE%.
) else (
  echo [PA] Restore helper finished.
)
echo.
pause
exit /b %CODE%
