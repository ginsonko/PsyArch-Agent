...
set "PA_SELF=%~f0"
set "PA_TEMP_PS=%TEMP%\pa_restart_update_%RANDOM%%RANDOM%.ps1"
set "PS_CMD="
...
echo ======================================
echo   PsyArch-Agent restart and update
echo ======================================
echo Folder: %cd%
echo.
echo This will:
echo   1. Stop the process using port 8765.
echo   2. Update this current folder in place.
echo   3. Start PA-Agent + AP backend again.
echo.
...
echo.
if not "%CODE%"=="0" (
  echo [PA] Restart/update helper exited with errorlevel=%CODE%.
) else (
  echo [PA] Restart/update helper finished.
)
echo.
pause
exit /b %CODE%
