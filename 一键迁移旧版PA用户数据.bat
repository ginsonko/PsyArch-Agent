...
echo ======================================
echo   PsyArch-Agent user data migration
echo ======================================
echo Folder: %cd%
echo.
echo This copies user data from an old PA folder into this folder.

echo Existing target data will be backed up first.
echo.
...
echo.
if not "%CODE%"=="0" (
  echo [PA] Migration helper exited with errorlevel=%CODE%.
) else (
  echo [PA] Migration helper finished.
)
echo.
pause
exit /b %CODE%
...
Write-Host "  4. If the old outputs folder was huge, run 清理日志和临时输�?bat in the old folder before keeping it."
